...
class BackgroundsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create background" do
    assert_difference('Background.count') do
      post backgrounds_url, params: { background: { description: @background.description, name: @background.name } }
    end

    assert_redirected_to background_path(Background.last)
  end
  ...
  test "should update background" do
    patch background_url(@background), params: { background: { description: @background.description, name: @background.name } }
    assert_redirected_to background_path(@background)
  end
